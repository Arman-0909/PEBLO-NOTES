...
export default function Dashboard() {
  ...
  if (!data) return (
    <div className="p-8 text-center text-red-500 font-bold text-xl">Oops! Failed to load data.</div>
  );
  ...
  return (
    <div className="space-y-10 py-6">
      <div className="bg-gradient-to-r from-purple-500 to-violet-400 p-8 md:p-10 rounded-[3rem] border-b-8 border-purple-700 shadow-lg text-white">
        <h1 className="text-4xl md:text-5xl font-black tracking-tight drop-shadow-md">Hello, {data.username}!</h1>
        ...
        <div className="card-base">
          ...
          {data.recent_notes.length === 0 ? (
            <div className="empty-state-dashed">
              <p className="text-purple-500 dark:text-purple-400 font-bold text-lg mb-2">Your backpack is empty!</p>
              ...
              {data.recent_notes.map(note => (
                <li key={note.id}>
                  <Link to={`/notes/${note.id}`} className="list-card block">
                    <h3 className="font-bold text-xl text-purple-900 dark:text-purple-100">{note.title}</h3>
            ...
            <div className="flex flex-wrap gap-4">
              {data.most_used_tags.map(({ tag, count }) => (
                <div key={tag} className="tag-pill">
                  ...
                  <span className="bg-amber-400 dark:bg-amber-600 text-amber-900 dark:text-amber-100 font-black text-sm px-3 py-1 rounded-xl">{count}</span>
                </div>
              ))}
            </div>
          )}
